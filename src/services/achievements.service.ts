...
export async function fetchAllAchievements(): Promise<Achievement[]> {
  const { data, error } = await supabase.from('achievements').select('*').eq('is_active', true).order('order_index', { ascending: true })
...
export async function fetchAchievementsWithStatus(userId: string): Promise<AchievementWithStatus[]> {
  const [all, userAch] = await Promise.all([fetchAllAchievements(), fetchUserAchievements(userId)])
...
async function awardAchievement(userId: string, a: Achievement): Promise<boolean> {
  const { error } = await supabase.from('user_achievements').insert({ user_id: userId, achievement_id: a.id, xp_awarded: a.xp_bonus })
  if (error && error.code !== '23505') { console.warn(`awardAchievement: ${error.message}`); return false }
  if (!error && a.xp_bonus > 0) await awardXP({ amount: a.xp_bonus, sourceType: 'achievement_unlock', sourceEntityType: 'achievement', sourceEntityId: a.id })
  return !error
}

export interface UserMetrics { lessonsCompleted: number; streakDays: number; wordsLearned: number; perfectQuizzes: number; currentLevel: number; totalXP: number }

export async function checkAndUnlockAchievements(userId: string, metrics: UserMetrics): Promise<Achievement[]> {
  const [all, userAch] = await Promise.all([fetchAllAchievements(), fetchUserAchievements(userId)])
