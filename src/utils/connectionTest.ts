...
interface TestResult { name: string; passed: boolean; message: string; data?: unknown }

async function test(name: string, fn: ()=>Promise<unknown>): Promise<TestResult> {
  try { const data = await fn(); return { name, passed:true, message:'✅ PASS', data } }
  catch(e) { return { name, passed:false, message:`❌ FAIL: ${e instanceof Error?e.message:String(e)}` } }
}
...
export async function runConnectionTest(): Promise<void> {
  ...
  results.push(await test('Fetch topics', async()=>{
    ...
    if (!data||data.length===0) throw new Error('No topics — run seed.sql')
    return data
  }))
  results.push(await test('Fetch lessons', async()=>{
    ...
    if (error) throw error
    if (!data||data.length===0) throw new Error('No lessons — run seed.sql')
    return data
  }))
  ...
  results.push(await test('Achievements (expect 5)', async()=>{
    const { data, error } = await supabase.from('achievements').select('id,slug').eq('is_active',true)
    if (error) throw error
  ...
  results.push(await test('Auth session check', async()=>{
    ...
    if (error) throw error
    return session?`Logged in as ${session.user.email}`:'Not logged in (anon)'
  }))
  results.push(await test('calculate_level() RPC', async()=>{
    const { data, error } = await supabase.rpc('calculate_level',{p_total_xp:150})
    if (error) throw error
    if (data!==2) throw new Error(`Expected 2, got ${data}`)
    return `calculate_level(150) = ${data}`
  }))
  results.push(await test('RLS: anon cannot read profiles', async()=>{
    const { data, error } = await supabase.from('profiles').select('id').limit(1)
    if (error) throw error
    if (data&&data.length>0) throw new Error('RLS VIOLATION!')
    return 'Anon = 0 rows ✅'
  }))

  console.log('\n📋 Results:')
  results.forEach(r=>{ console.log(`  ${r.message}  ${r.name}`); if(r.data&&r.passed)console.log('    Data:',r.data) })
  const passed = results.filter(r=>r.passed).length
  console.log(`\n🏁 ${passed}/${results.length} tests passed`)
  if(passed===results.length) console.log('🎉 All tests passed!')
  else { console.warn('⚠️  Some tests failed.'); console.log('1. Check .env\n2. Run schema.sql\n3. Run seed.sql\n4. Restart dev server') }
  console.groupEnd()
}
